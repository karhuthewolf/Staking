...
mod ERC20 {
    ...
    fn mint(recipient: ContractAddress, amount: u256) {
        let caller = get_caller_address();
        assert(_minters::read(caller), 'ERC20: caller is not a minter');
        _mint(recipient, amount);
    }

    #[external]
    fn setMinter(account: ContractAddress, is_minter: bool) {
        let caller = get_caller_address();
        assert(caller == _owner::read(), 'ERC20: caller is not the owner');
        _minters::write(account, is_minter);
    }
